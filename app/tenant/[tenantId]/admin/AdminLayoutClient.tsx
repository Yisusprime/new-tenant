...
function AdminLayoutContentInner({
  ...
  useEffect(() => {
    ...
    if (isMobile) {
      ...
      // En escritorio, ocultamos automáticamente solo en rutas específicas
      setSidebarOpen(!shouldAutoHideSidebar())
    }
  }, [pathname, isMobile])
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      <div
        className={`md:hidden fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform transition-transform duration-300 ease-in-out ${
      ...
      {sidebarOpen && (
        <div className="hidden md:block w-64 bg-white shadow-lg">
          ...
          {/* Contenido del sidebar escritorio */}
          {renderSidebarContent()}
        </div>
      )}
  ...
  // Función para renderizar el contenido del sidebar (usado en ambas versiones)
...
function AdminLayoutClient({ children, params }: AdminLayoutProps) {
  return (
    <BranchProvider tenantId={params.tenantId}>
      <PlanProvider tenantId={params.tenantId}>
        <AuthProvider>
          <AdminLayoutContentInner children={children} params={params} />
        </AuthProvider>
      </PlanProvider>
    </BranchProvider>
  )
