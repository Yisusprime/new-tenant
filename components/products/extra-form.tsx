...
export function ExtraForm({ productId, extraId, onCancel }: ExtraFormProps) {
  const { products, addExtra, updateExtra, tenantId } = useProducts()
  ...
  useEffect(() => {
    if (extraId) {
      ...
      if (product && product.extras && product.extras[extraId]) {
        const extra = product.extras[extraId]
        setName(extra.name)
